        ...
        * @param {string} [suffix=</b>] - The string to insert at the end of a highlight. */
        address.highlight = function (suggestions, prefix, suffix) {
            ...
            for (var s = 0; s < suggestions.length; s++) {
                ...
                var highlights = suggestion.Highlight.split(",");
...
(function() {
    ...
    /**
    * Capture Plus bindings object.
    * @typedef {Object} pca.CapturePlus.Bindings
    ...
    * 
    * 
    * Platform scripts are aimed are specially designed only to be served from the tag platform
    * V2.00 is FUZZY
    * 
    */
    
    /** Automatically sets up pca.Address controls for Capture Plus.
    * @memberof pca 
    * @constructor
    * @mixes Eventable */
    pca.CapturePlus = function () {
        /** @lends pca.CapturePlus.prototype */
        /** @lends capturePlus.prototype */
        var captureplus = new pca.Eventable(this);
       
        /** The list of address controls.
        * @type {Array.<pca.Address>} */
        captureplus.controls = [];

        //add options from account and set defaults
        function refactorOptions(options, key) {
            options = options || {};
            options.countries = options.countries || {};
            options.search = options.search || {};

            if (options.CapturePlusCountryByIP) options.setCountryByIP = !!options.CapturePlusCountryByIP;
            if (options.CapturePlusDefaultLanguage) options.culture = options.CapturePlusDefaultLanguage;
            if (options.CapturePlusDefaultCountry) options.countries.defaultCode = options.CapturePlusDefaultCountry;
            if (options.CapturePlusCountryList) options.countries.codesList = options.CapturePlusCountryList;

            if (options.CapturePlusCountryList) options.search.countries = options.CapturePlusCountryList;
            if (options.CapturePlusDefaultLanguage) options.search.language = options.CapturePlusDefaultLanguage;
            ...
            //captureplus.fire("options", options);
            pca.platform.fire("options", ["capture+", key, options]);

            return options;
        }

        captureplus.bindings = pca && pca.platform && pca.platform.getBindingsForService ? pca.platform.getBindingsForService("PLATFORM_CAPTUREPLUS") : [];
        ...
        * @fires ready
        */
        captureplus.load =  function() {
            captureplus.destroy();
            
            for (var i = 0; i < captureplus.bindings.length; i++) {
                var key = "";
                try { key = captureplus.bindings[i].options.key; } catch (ex) { }
                pca.platform.fire("fields", ["capture+", key, captureplus.bindings[i].fields]);
                var control = new pca.Address(captureplus.bindings[i].fields, refactorOptions(captureplus.bindings[i].options, key));

                control.listen("populate", function(address, variations) {
                    pca.platform.fire("data", ["address", control.key, address, variations]);
                });

                captureplus.controls.push(control);
                pca.platform.fire("load", ["capture+", control.key, control]);
            }
            pca.platform.fire("ready", ["capture+", captureplus]);
        }

        /** Applies a delegate function to all controls.
        * @param {pca.CapturePlus~controlDelegate} delegate - The delegate function to handle each control. */
        captureplus.all = function(delegate) {
            for (var i = 0; i < captureplus.controls.length; i++)
                delegate(captureplus.controls[i]);
            
            /** Delegate function to handle a control.
            * @callback pca.CapturePlus~controlDelegate
            * @param {pca.Address} control */
        }

        /** Sets the culture of all controls.
        * @param {string} culture - The culture code to set. */
        captureplus.setCulture = function(culture) {
            captureplus.all(function(c) { c.setCulture(culture); });
        }
        
        /** Sets the country of all controls.
        * @param {string} country - The country name or code to change to. */
        captureplus.setCountry = function(country) {
            captureplus.all(function(c) { c.setCountry(country); });
        }
        
        /** Sets the width of all controls.
        * @param {number|string} width - The width in pixels for the control. */
        captureplus.setWidth = function(width) {
            captureplus.all(function(c) { c.setWidth(width); });
        }
        
        /** Sets the height of all controls.
        * @param {number|string} height - The height in pixels for the control. */
        captureplus.setHeight = function(height) {
            captureplus.all(function(c) { c.setHeight(height); });
        }
       
        /** Enables all controls. */
        captureplus.enable = function() {
            captureplus.all(function(c) { c.enable(); });
        }
        
        /** Disables all controls. */
        captureplus.disable = function() {
            captureplus.all(function(c) { c.disable(); });
        }

        /** Destroys all controls. */
        captureplus.destroy = function() {
            captureplus.all(function(c) { c.destroy(); });
            captureplus.controls = [];
        }

        //Initialise Capture Plus.
        pca.ready(captureplus.load);
    }
